...
Админка для мониторинга креаторов TikTok и Instagram: список с тегами, карточка креатора со статистикой за срок, сравнение видео с медианой аккаунта. Один пользователь — владелец.

Сайт — **статика на GitHub Pages**, сервера нет: страницы ходят в Supabase прямо из браузера (`supabase-js`, публичный ключ), доступ к данным решает RLS.

Цифры в базу приносит **сборщик — `collector/`**, отдельный Node-пакет в этом же репозитории. Он работает на домашнем компьютере владельца: водит установленный браузер по профилям креаторов, забирает Instagram через официальный Graph API и пишет снимки в ту же базу ключом `service_role`. Обходит трижды в день (10:00, 13:00, 17:00), догоняет пропущенный слот после сна компьютера и слышит кнопку «Обновить» на сайте через `sync_requests`. Как поставить, настроить и что делать, когда TikTok отдаёт пустой список видео, — в [`collector/README.md`](collector/README.md).
