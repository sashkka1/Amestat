...
export function SyncButton({
  ...
  // Просьба жива, пока не кончился обход, — но сборщик, который не отозвался, кнопку
  // ОСВОБОЖДАЕТ (владелец, 2026-09-14: «не нужно было лочить кнопку… если кто-то хочет
  // пытаться, пусть пытается; я уже когда приду, запущу сервер»). Прежде замок держался до
  // конца обхода, и человек, получивший «сборщик не отвечает», не мог больше ничего сделать
  // вовсе. Очередь одинаковых просьб от этого не страшна: сборщик сводит их в один обход
  // (`groupRequests` в collector/requests.mjs).
  // ⚠️ `late` здесь наравне с `notified`: это тот же немой сборщик, только бот не настроен
  // и письма не было.
  const waiting = phase !== "idle" && !notified && !late;
