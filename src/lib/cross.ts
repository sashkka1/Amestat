// Перекрёстность креаторов: что из строк `cross_stats` (миграция v29) показывают дашборд и
// карточка креатора. Здесь только расчёты — ни запросов, ни разметки: строки приходят из
