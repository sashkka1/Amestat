...
// Насколько поздно после отметки ещё можно снять «просмотры на отметке». Владелец,
// 2026-09-19: «если видео опубликовано 48 часов назад и окно 48 часов — плюс-минус час, — я
// знаю, что окно закрывается прямо сейчас; не нужно было 48 часов назад его трекать».
// Просмотры накопительные: снимок на 48-м часу и есть «просмотры за 48 часов», когда бы мы
// ни начали смотреть видео.
export const MARK_TOLERANCE_MS = 60 * 60_000;

// Знаем ли мы просмотры на отметке. Да — если сборщик видел видео внутри окна (тогда окно
// закроет снимок на отметке или после неё) ИЛИ если первый снимок после отметки сделан не
// позже чем через час после неё. Нет — только когда мы считаем задним числом: впервые
// увидели видео заметно позже отметки и что было на ней, не знаем (владелец: «это правило
// актуально, только если мы считаем задним числом… а если статистика есть — не нужно»).
export function measurable(stat: PaymentStat): boolean {
  if (stat.has_early) return true;
  if (stat.window_at === null) return false;
  return Date.parse(stat.window_at) - Date.parse(stat.mark_at) <= MARK_TOLERANCE_MS;
}

export function videoMoney(stat: PaymentStat, rules: PaymentRules): VideoMoney {
  // Просмотров на отметке не знаем — платить не за что (владелец, 2026-09-19: «оставить без
  // выплат вообще… помечать, что выплат нет»). Так стоят ролики, вышедшие до того, как
  // сборщик начал их снимать: площадка отдаёт только текущее число, а видео впервые попало к
  // нам через сотни часов после отметки. В порог `videos_threshold` оно при этом входит:
  // видео сделано.
  if (!measurable(stat)) {
