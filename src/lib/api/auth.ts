...
export async function login(loginOrEmail: string, password: string): Promise<ActionResult> {
  ...
  if (!raw || !password) return fail(tr("api.authEnterBoth"));
  ...
  if (error) {
    return fail(
      error.code === "invalid_credentials" || error.status === 400
        ? tr("api.authBadCredentials")
        : tr("api.authLoginFailed", { message: error.message }),
    );
  }
...
const INVITE_REASON_KEY: Record<
  Exclude<InviteCheck, "ok">,
  "invites.reasonInvalid" | "invites.reasonUsed" | "invites.reasonExpired"
> = {
  invalid: "invites.reasonInvalid",
  used: "invites.reasonUsed",
  expired: "invites.reasonExpired",
};

export function inviteReason(check: Exclude<InviteCheck, "ok">): string {
  return tr(INVITE_REASON_KEY[check]);
}
...
export async function checkInvite(token: string): Promise<ActionResult<InviteCheck>> {
  ...
  if (error) return fail(tr("api.authCheckInviteFailed", { message: error.message }));
...
export async function isLoginTaken(login: string): Promise<ActionResult<boolean>> {
  ...
  if (error) return fail(tr("api.authCheckLoginFailed", { message: error.message }));
...
export async function registerByInvite(input: {
  ...
  if (!loginText) return fail(tr("api.authEnterLogin"));
  if (input.password.length < 8) return fail(tr("api.authPasswordShort"));

  const supabase = createClient();

  const taken = await isLoginTaken(loginText);
  if (!taken.ok) return taken;
  if (taken.data) return fail(tr("api.authLoginTaken"));
  ...
  if (error) return fail(tr("api.authRegisterFailed", { message: error.message }));
  ...
  if (!data.session) {
    ...
    if (signedIn.error)
      return fail(tr("api.authRegisteredNoLogin", { message: signedIn.error.message }));
  }
  return { ok: true, data: undefined };
}
