...
//
// ⚠️ Слот ОДИН — 16:00 (владелец, 2026-09-10: утренний выключен совсем), и стоит он в
// `.env.local` (`AMESTAT_SLOTS=16`). Умолчание кода на случай пустой настройки — то же 16:00,
// чтобы пустая переменная не воскрешала прежние 13:00. Утренний срез теперь даёт не слот, а
// первый обход дня от старта компьютера и от пробуждения из сна (`firstRunAt`).
//
...
// включился, ~5 минут на разогрев — и обход». Тем же правилом резидент планирует обход после
// сна: машина могла проснуться утром нового дня.

/**
 * Момент первого обхода дня: `now` плюс `minutes` (AMESTAT_START_DELAY_MIN, по умолчанию 5).
 * ⚠️ `0` здесь законен и означает «сразу» — в отличие от повтора ручной просьбы, где ноль
 * значил бы обход в ту же секунду, что и неудача. Мусор и отрицательное — умолчание.
 * Чистая функция: её проверяют тесты.
 */
export function firstRunAt(now, minutes = 5) {
  const min = Number.isFinite(minutes) && minutes >= 0 ? minutes : 5;
