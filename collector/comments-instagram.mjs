...
// «Просмотреть» — на вырост: TikTok сменил «Посмотреть» на «Просмотреть» без предупреждения
// (2026-09-08), и та же замена у Instagram иначе сломала бы раскрытие тем же способом.
const BRANCH_OPEN = { selector: null, text: "^(?!.*(ещё|еще))(просмотреть|смотреть|посмотреть|показать)\\s+(все\\s+)?\\d*\\s*ответ|^view\\s+(all\\s+)?\\d*\\s*repl" };
